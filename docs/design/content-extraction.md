...
- 用户点击 `提取文案` 后，后端同步执行文案提取并返回结果：
  ...
  - 图文：按下载图片顺序逐张调用 SiliconFlow 视觉理解能力提取漫画页内容，再调用 SiliconFlow 文本 LLM 整理为最终内容提取结果。
...
5. 如果 `media_type=gallery`：
   ...
   - 逐张调用 SiliconFlow 视觉理解模型提取漫画页内容。
   - 保存每张图片的视觉识别原始结果。
   - 合并全部逐页结果后，调用 SiliconFlow 文本 LLM 整理最终内容提取结果。
6. 同步返回最终内容提取结果和每个媒体项的逐页识别结果。
...
- 图文二次整理：把全部逐页识别结果作为输入，最终输出写入内容提取详情的主结果区。
- 视频：保存一段完整转录文本。
- 每次模型原始输出保存到内容提取媒体记录，便于排查。
