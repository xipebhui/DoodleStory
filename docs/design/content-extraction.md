...
- 用户点击 `提取文案` 后，后端同步执行文案提取并返回结果：
  ...
  - 图文：按下载图片顺序逐张提交，每次请求只包含一个公网原图 URL；模型只提取当前图片实际可见、可用于复刻的画面与原文文字，不理解连续故事。每张图片先调用 `TEXT_FALLBACK_*` 当前指向的火苗平台，失败后切换 `LIO_*` 并最多发起 3 次 LIO 请求。
...
- 每张图片的火苗主平台请求次数：`1` 次，写在 `media_text_extraction.py` 中。
- 每张图片切换 LIO 后的最大请求次数：`3` 次，重试退避基数为 `2` 秒，写在 `media_text_extraction.py` 中。
...
- 图文内容提取主平台失败后使用现有 `LIO_API_KEY`、`LIO_BASE_URL` 和 `LIO_MODEL`；LIO 配置缺失时明确失败，不继续无意义重试。
...
5. 如果 `media_type=gallery`：
   ...
   - 严格顺序逐张调用视觉理解模型，每次请求只提交当前图片的一个公网 `image_url`。
   - 模型只提取当前图片实际可见的分格、画面、原文文字和文字布局，不读取或推断前后页故事。
   - 当前火苗主平台单图失败时，对同一张图片切换 LIO，最多发起 3 次 LIO 请求。
   - 后端给每份成功结果确定性添加 `第X页`，再按 `display_order` 合并，不依赖模型生成页码。
   - 任意图片在 LIO 3 次均失败后终止整项提取，不跳过图片或保存占位结果。
   - 全部单图成功后，把合并内容保存为最终内容提取结果。
6. 同步返回最终内容提取结果。
...
- 火苗单图理解失败后允许按用户授权切换 LIO；LIO 最多 3 次仍失败，或 SiliconFlow 音频转写失败时，提取请求失败，不返回占位文案。

## 模型调用

图文内容提取通过 OpenAI 兼容 `/chat/completions` 逐图调用，消息 `content` 每次只包含单图 prompt 和一个 `image_url`。主平台读取 `TEXT_FALLBACK_*`，失败后的 LIO 读取现有 `LIO_*`；两者使用同一个单图 prompt。视频音频转写仍通过 SiliconFlow 多模态模型调用，消息 `content` 使用 `audio_url`。

媒体输入策略：

- 图文图片必须先登记为 DoodleStory 资产，并把对象存储公网原图 URL 按顺序逐张传给当前视觉平台；没有公网 HTTP(S) URL 时明确失败。
...
```text
只提取当前这一张图片中实际可见、可用于复刻画面的内容，不推断前后故事，不补写上下文，不总结或改写剧情。
...
- 图文：每张图片独立返回固定结构，后端添加连续 `第X页` 页标并按原始顺序合并后，写入内容提取详情的主结果区。
- 视频：保存一段完整转录文本。
- 每次模型原始输出写入后端日志，便于排查。
