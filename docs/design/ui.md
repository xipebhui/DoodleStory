...
- 风格：可搜索选择器，展示风格缩略图、名称和简短描述。模型配置不作为单独模块展示。
...
- 描述：选填。
- 状态：草稿或启用。
- 图片 provider key：必填，作为风格内部配置。
- 图片 model key：必填，作为风格内部配置。
- 模型默认参数：选填，结构化 JSON 或表单化参数。
- 风格提示词：必填，多行文本框。
- 参考图片：选填，支持多图上传。
...
- 风格提示词按作者输入保存。
- 图片模型配置在风格提示词附近明确展示，但不提供独立模型管理页面。
- 删除已保存的参考图需要确认。
...
- 名称、状态、描述。
- 图片 provider key、model key 和默认参数。
- 参考图片画廊。
- 风格提示词。
- 最近风格测试记录。
- 使用该风格的任务。
