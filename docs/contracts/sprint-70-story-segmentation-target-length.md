...
修正完整故事自动切割过碎、过僵硬的问题。LLM 切割不要求逐字覆盖原文，但应尽量保留原文，并以画面单元、情绪转折和叙事节奏为首要标准，30-40 字只作为次级偏好。同时修复最终生图提示词中出现“第几页”页码，导致单张图片把页码角标画进画面的问题。

## 范围内

- 完整故事 `segment_story` 调用传入目标长度范围 `target_panel_text_chars=30-40`。
- 完整故事首轮语义切分和碎片化二次合并调用 LIO/Google 文本模型，避免继续被默认 DeepSeek/SiliconFlow 文本模型的长耗时和切分不稳定性影响。
- 切割 prompt 明确要求自动数量模式以画面单元、情绪转折和叙事节奏为首要目标，不能为了凑 30-40 字硬合并两个本应分开的画面动作或情绪转折。
...
- 不恢复逐字覆盖校验。
- 不改变故事方案模式和提取分镜模式。
- 不改变角色提取、故事方案规划、最终生图 prompt 编译等其他 LLM 调用模型。
...
- LLM 切割输入中包含目标长度范围 30-40，并单独传入 50 字后端硬校验上限。
- 完整故事切分和二次合并只调用 LIO/Google JSON 文本接口。
- Prompt 明确语义和画面节奏优先于长度目标，避免硬凑 30-40 字。
...
- 30 字是软目标，不是硬校验；40 字是提示词生成目标上限，用于给空格、标点、数字和英文留余量；50 字仍是后端硬校验上限。少数自然短句、强转折或结尾余句仍可保留短 panel。二次合并仍由 LLM 完成，不引入本地兜底切割。
