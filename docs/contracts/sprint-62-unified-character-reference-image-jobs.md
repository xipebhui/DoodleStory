...
- 长故事与入口统一已纳入本 Sprint 的后续修正：完整故事不再使用程序断句 chunk，改为 LIO 文本 LLM 生成逐字覆盖原文的 storyboard panels 和 continuity_plan；故事方案、提取分镜和 DY 爆款复刻继续统一进入页式分镜中间态。最终生图 prompt 编译按 `LLM_PANEL_BATCH_SIZE` 分批调用，默认 10，并携带 compact storyboard context。该变更不改数据库结构，也不改变图片 job 队列。
