...
| 7 | 完整 Evaluation、稳定性、成本和发布门槛 | 未开始 | 阶段 6 后创建合同 |
...
## 8. 阶段 4：Panel 迭代与 VL 闭环

阶段 4 已建立 `docs/contracts/sprint-108-agent-panel-iteration-vl-draft.md`，只有阶段 3 通过后才能评审激活。
...
每增加一种资源都必须覆盖归属校验、快照、对话恢复和目标局部性。阶段 5 完成时，不同入口只是向同一 Agent Turn 注入不同资源，而不是启动不同 Pipeline。

## 10. 阶段 6：旧 Pipeline 迁移与删除
...
阶段 6 的完成定义是“创作决策只有 Agent 一套”，不是“旧文件暂时没人点”。

## 11. 阶段 7：Evaluation 与发布
