...
- 统一四种创建入口的前置分镜链路：完整故事不再使用程序断句 chunk，改为调用 LIO 文本 LLM 做完整故事 storyboard planning，输出逐字覆盖原文的 panels、画面 prompt、文字结构和 continuity_plan；后端强校验所有 `story_beat` 拼接必须等于用户原文。故事方案和提取分镜继续走各自 LLM storyboard planning，DY 爆款复刻先做内容提取后进入提取分镜链路。完整故事新任务不再单独创建 `generate_panel_prompts` 步骤；角色提取在分镜后读取 panels 并保存 appearance 到 panel 的绑定；最终 prompt 编译输入新增 compact `storyboard_context`，用于约束时间线、连续场景、对白说话人和人物年龄阶段。
