...
- `docs/contracts/sprint-62-unified-character-reference-image-jobs.md`

## 最近完成的工作

- 优化长故事任务的文本 LLM 上下文：完整故事模式的人物提取现在只输入原始故事全文，不再把所有切分 panels 重复传入；完整故事 panel 画面设计按 `LLM_PANEL_BATCH_SIZE` 分批调用，只传当前批次 panels 和相邻 panel 短上下文；最终生图 prompt 编译也按同一批大小分批处理，不再携带完整原文或 story_context，但每批都会携带全局角色表、风格信息和参考图顺序，继续由编译层统一处理角色一致性、风格冲突和文字呈现。默认批大小为 10，可通过环境变量调整。
