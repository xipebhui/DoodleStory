...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 保持原文切分；`故事方案` 直接识别用户方案中的显式或隐式 panel 逻辑，并规划封面、剧情图、画面内容、图片内文字和文字排版。
- Panel：由用户原始故事或故事方案规划出的画面片段。Panel 保存剧情意图、类型、图片内文字、文字布局和生图画面 prompt。
- 封面：故事方案模式的第一个 panel，包含故事标题、钩子和核心视觉画面。
- 人物参考：任务级可选能力。开启后，系统会在切分故事后识别主要人物及其不同年龄/外形阶段，先生成对应人物参考图，再把相关人物参考图按固定顺序传入每个 panel 的生图请求。
...
4. 执行任务生成。
   - 如果任务是故事方案模式，调用 LLM 直接做图文 storyboard planning，输出标题、钩子、规划概要、每个 panel 的剧情意图、画面 prompt、图片内文字和文字布局。
   ...
   - 如果任务开启人物参考，调用 LLM 提取主要人物及其不同年龄/外形阶段。只识别对连续画面一致性有价值的主要人物，不识别路人、群众或一次性背景人物。
   - 开启人物参考时，先使用任务风格生成每个人物外形阶段的参考图。人物参考图只用于后续 panel 生图，不向用户展示内部人物提示词。
   - 完整故事模式下，再次调用 LLM 输入原始故事和 panel 文本，生成每张图的图文设计：`visual_prompt`、`image_text` 和 `text_layout`。所选风格提示词进入该 LLM 调用的 system prompt，作为图文设计规则使用，不再作为普通 user 字段传入。
   - 故事方案模式下，图文设计已经在 storyboard planning 中生成，不再额外走 chunk 后的 panel prompt 生成。
   - 开启人物参考时，完整故事模式的 panel prompt 生成还需要标注当前 panel 涉及哪些人物外形阶段；故事方案模式使用人物提取结果中的 panel_orders 绑定参考图顺序。
   ...
   - 最终发送给图片模型的 panel 生图 prompt 不再拼接原始风格提示词，而是使用已风格化的图文设计，并按 `参考[]`、`场景[]`、`人物和对话[]`、`图片文字[]`、`排版[]` 组织。
