...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 保持原文切分；`故事方案` 直接识别用户方案中的显式或隐式 panel 逻辑，并规划封面、剧情图、画面内容和图片内文字。故事方案中用户明确写出的画面、构图、文字、重点和禁止事项优先于风格规则、默认分镜方法和剧情增强建议。
...
- 内容提取：用户粘贴抖音分享文本或链接，系统从中解析真实抖音 URL，下载图文或视频；图文按图片顺序用 SiliconFlow 视觉理解模型逐页完整提取漫画内容，再调用 SiliconFlow 文本 LLM 整理为最终内容提取结果；视频通过音频多模态模型转写；图文内容还可以基于全部图片生成故事总结。
...
3. 创建生成任务。
   - 用户注册或登录。
   - 通过居中弹窗创建任务。
   - 选择输入模式：完整故事或故事方案。完整故事会保持故事不变，用户应只提交故事本身，不混入说明、标签或额外要求；故事方案可以提交故事设计、人物设定、画面要求或简化想法。
   - 输入原始文本、故事方案或创作要求。
...
6. 提取抖音内容文案。
   ...
   - 后端随后在同进程后台处理该记录：调用同机抖音下载服务下载图文或视频，并把下载媒体登记为 DoodleStory 资产；下载媒体登记完成后必须先提交数据库，让详情页可以在后续内容提取和故事总结仍在处理时先展示图片。
   - 如果作品是视频，后端从下载服务返回的本地原始视频路径分离音频，再调用 SiliconFlow 音频多模态能力转录原始口播、旁白或对白。
   - 如果作品是图文，后端按图片顺序逐张使用下载服务返回的本地原始图片路径调用 SiliconFlow 视觉理解模型，按漫画页完整提取旁白、对话、内心 OS、画面描述和分格信息；逐页识别完成后再调用 SiliconFlow 文本 LLM 整理最终内容提取结果，并再次提交数据库，让详情页可以在故事总结仍在处理时先展示内容提取结果。
   - 如果作品是图文，后端把下载服务返回的本地原始图片按顺序输入视觉理解模型生成故事总结，输出 `故事内容`、`故事爆点` 和 `目标观众`。
   - 页面以内容提取任务列表为入口；创建任务使用弹窗，详情查看使用弹窗。
   - 任务完成后不自动打开详情；用户从列表点击任务行查看详情。
   - 列表页只加载内容提取摘要，不加载所有图片；完整媒体只在打开详情后按需加载。
   - 详情页以最终文案和故事总结为主；下载后的媒体只作为辅助预览，多图默认折叠。
...
- 内容提取模型：图文图片内容提取使用 SiliconFlow 多模态视觉模型，通过 `/chat/completions` 传入 `image_url` 内容，逐页输出旁白、对话、内心 OS、画面描述和分格信息；逐页结果合并后再调用 SiliconFlow 文本 LLM 生成最终内容提取结果。图文故事总结和视频音频转写继续使用 SiliconFlow 多模态模型，图文故事总结输入按下载顺序排列的全部图片，视频第一版先分离音频后转写，不直接做视频画面理解。内容提取、故事总结和视频转写必须使用抖音下载服务返回的本地原始媒体路径，不应为了处理流程从对象存储公开 CDN 回拉刚下载的媒体。
- 内容提取创建使用轻量后台处理：提交后立即保存记录并返回列表，后台分阶段完成解析下载、内容提取和图文故事总结；下载媒体登记后先提交，内容提取完成后再次提交，故事总结完成后标记成功。列表按需刷新处理状态，仍保留显式重新提取、重新总结接口用于用户在详情中重新执行，不引入外部队列或复杂状态机。
...
- 内容提取必须保存用户粘贴的原始输入和后端解析出的 URL；图文内容提取必须逐页保留图片中可见的旁白、对话和内心 OS 原文，不做总结、润色、扩写或故事化改写。
