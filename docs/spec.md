...
- Agent 当前真实基线由 Sprint 105–114 完成：除 Conversation/Message/Run/Step 外，新增版本化 `agent_artifacts`、hash 绑定 `agent_approval_requests` 和用户安全 `agent_events`；`agent_runs.task_id` 继续关联同一 `generation_tasks`。正式漫画创建加载 `idea-to-comic`，生成 2–8 Panel 方案并停在 `waiting_for_input`，owner 批准后才创建 Task/Panel/Image Job；页面通过 SSE cursor 读取持久化事件，不再轮询 Agent 进度。资源输入当前仍只解析一个 Style，结构化 Character/Task/Panel/Image Version 引用由后续 Sprint 扩展。
