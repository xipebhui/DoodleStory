...
- Agent V1 Runtime 基础已完成：创意扩写、分镜规划、生图提示词、结果检查和重试决策仍由单个漫画导演 Agent 逐阶段接管；Runtime 负责权限、状态、预算、幂等和可靠性，不把旧 pipeline 的 prompt 拼接步骤原样包装成 Tool。阶段 1 已创建 `agent_conversations`、`agent_messages`、`agent_runs` 和 `agent_steps` 四张表，并提供 `/api/v1/agent/conversations`、Conversation 详情/发消息与 `/api/v1/agent/runs/{run_id}` API；普通用户和 Admin 都只能通过这些入口访问自己的 Agent 数据。消息资源引用先以受控 JSON 保存，本阶段不解析 `@风格/@角色/@任务/@Panel/@图片版本`。进程内 Agent 队列只传 `run_id`，数据库保存完整模型 Step 和最终消息 checkpoint；服务重启时恢复 `queued/running/retrying` Run，已成功模型 Step 只补写最终消息，不重复请求模型。正式 Runtime 锁定 `openai-agents==0.18.3`、`openai==2.45.0`、Responses API 和统一模型 `gpt-5.6-terra`；火苗 `TEXT_FALLBACK_*` 为主平台，LIO `LIO_*` 为备用平台，底层 client/SDK retry 均关闭。Router 只对连接、超时、429 与语义明确的临时 5xx 在火苗重试一次，仍失败时切换一次 LIO；401/403、schema、内容策略、`invalid_request`、`model_not_found`、无渠道或能力错误明确失败。每次模型输入从应用数据库完整重放，不使用 Provider `previous_response_id` 或 remote conversation。阶段 1 的 Agent 只返回真实文本，不调用漫画 Tool、不创建 GenerationTask；现有生产生成 pipeline 保持不变，后续按独立 Sprint 迁移。
