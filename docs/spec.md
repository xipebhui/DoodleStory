...
- 任务：用户发起的一次文本转图片请求。
- 音频参考：用户上传并管理的参考音频资产，用于后续视频任务的旁白音色或参考音频选择。音频参考按用户隔离，普通用户只能看到自己的音频参考。音频参考创建时由后端本地 Whisper 自动从上传文件转写参考文本，并统一转换为简体中文；用户不手动填写参考文本、Provider、模型或音色名。用户可以为音频参考设置产出语速，后续视频任务会在创建时快照该语速。
...
4. 创建生成任务。
   ...
   - 可选勾选 `去掉画面文字`；默认不勾选。勾选后不改变任务分镜、旁白、对白或图片文字结构化字段，只在最终发送给图片模型的生图提示词最前面拼接 `最高指令，图片中不能包含任何文字。`。
   - 选择风格。
