...
- 人物参考：任务级可选能力。当前默认只使用用户显式绑定的固定角色；任务创建会把角色资产快照为任务内人物参考，并把相关人物参考图按固定顺序传入每个 panel 的生图请求。未绑定角色只保留在故事文本中，不自动生成参考图。
...
3. 管理角色。
   - 用户进入 `角色管理` tab。
   - 创建角色时填写名字并上传参考图；上传参考图后后端调用 SiliconFlow 视觉模型自动生成可编辑的角色外观描述，并保存到角色 `description`。
...
4. 创建生成任务。
   ...
   - 只有用户显式绑定了参考图的角色会进入人物参考链路；未绑定角色只保留在故事文本中，不自动生成参考图。
...
5. 执行任务生成。
   ...
   - 如果任务绑定了用户固定角色，后端把用户角色名字、描述和参考图快照保存为任务内人物参考；固定角色参考图不重新生成，不额外扣人物参考图积分。
   ...
   - 旧的自动人物参考能力不再作为创建任务默认行为；未绑定角色不自动生成参考图。
   ...
   - 开启人物参考时，完整故事模式的 panel prompt 生成还需要标注当前 panel 涉及哪些固定角色；故事方案和提取分镜模式使用确定性名字匹配绑定参考图顺序。
