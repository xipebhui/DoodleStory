...
- 内容提取第一版使用同步请求：创建任务时一键完成解析下载、文案提取和图文故事总结；仍保留显式重新提取、重新总结接口用于用户在详情中重新执行，不设计异步状态机、worker、轮询或取消流程。
