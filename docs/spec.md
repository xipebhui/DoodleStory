...
4. 执行任务生成。
   ...
   - 开启人物参考时，先使用任务风格提示词生成每个人物年龄阶段的参考图；最终发送给图片模型的人物参考图 prompt 必须把 `style_prompt_snapshot` 作为独立风格提示词段显式拼接，强化人物画风一致性。人物参考图只用于后续 panel 生图，不向用户展示内部人物提示词；风格参考图不参与人物参考图生成。
