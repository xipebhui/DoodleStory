...
- Agent V1（已设计、尚未实现）：创意扩写、分镜规划、生图提示词、结果检查和重试决策统一交给单个漫画导演 Agent；Runtime 只向模型暴露最基础的生图和 VL 检查工具，并负责权限、状态、预算、幂等和可靠性，不把旧 pipeline 的 prompt 拼接步骤原样包装成 Tool。Agent 对话、Run、Step、Tool Call/Output 和 `@风格/@角色/@任务/@Panel/@图片版本` 引用由 DoodleStory 数据库保存，用户关闭页面或服务重启后可从完整 step checkpoint 继续，不假设用户必须一直停留在同一对话框。模型线路采用火苗 `TEXT_FALLBACK_*` 作为主平台、LIO `LIO_*` 作为备用平台，两个平台的 Agent 模型统一为 `gpt-5.6-terra`；2026-07-22 更新 LIO API key 后，实测确认两边 Chat Completions、JSON、Chat Function Calling/Tool Output、多模态和基础 Responses 文本均通过。正式 API shape 由 Sprint 105 的 OpenAI Agents SDK Responses Tool Loop 实测锁定，不依赖 Provider response ID 保存上下文，也不在同一 workflow 混用两种 API shape。跨平台切换由 DoodleStory Router 执行，且只允许在尚未输出部分结果或执行不可判定副作用的可重试错误上发生；永久请求、权限、schema、策略、`model_not_found` 或能力错误必须明确失败。正式 Agent 落地前，当前生产生成 pipeline 保持不变，后续按全局路线的独立 Sprint 迁移，不在同一改动中大范围替换。
...
- Agent Runtime 的具体 SDK 和 API shape 由 Sprint 105 的真实 SDK Tool Loop 锁定；不得因为引入 SDK 改变应用侧持久化和 Router 错误分类契约。
