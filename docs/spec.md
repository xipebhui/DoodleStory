...
4. 执行任务生成。
   ...
   - 开启人物参考时，先使用任务风格提示词生成每个人物年龄阶段的参考图。人物参考图只用于后续 panel 生图，不向用户展示内部人物提示词；风格样张参考图不参与人物参考图生成。
   ...
   - 每个 panel 生图请求不传入风格样张参考图。未开启人物参考时不携带参考图；开启人物参考时只按固定顺序传入该 panel 绑定的人物参考图，并在最终 prompt 中用极简格式声明“人物参考（参考图 N）”。
...
- 外部集成：LLM 固定使用 SiliconFlow 配置；生图主路径使用 `docs/api_v3.md` 中已同意的 OpenAI Images 兼容统一服务，生图 API key 和 base url 从 `IMAGE_GATEWAY_API_KEY`、`IMAGE_GATEWAY_BASE_URL` 读取。当前可用生图模型精确限定为 `gpt-image-2`、`Tongyi-MAI/Z-Image`、`Qwen/Qwen-Image`、`baidu/ERNIE-Image-Turbo`、`gemini_3.1_flash_image_preview`、`gemini_3.0_pro_image_preview`、`gemini_3.1_flash_image_preview_4K`、`gemini_3.0_pro_image_preview_4K`、`gemini-3.1-flash-image-preview` 和 `gemini-3-pro-image-preview`。主路径统一请求 `/v1/images/generations`，返回的 `data[0].url` 或 `data[0].b64_json` 必须立即下载或解码并保存为 DoodleStory 资产；未列入清单的模型必须明确报错。若统一生图 Gateway 按既有重试策略最终仍发生 Provider 响应错误，允许显式切换到 XG 备用生图：没有参考图时调用 XG `/v1/images/generations`，有参考图时调用 XG `/v1/images/edits`，多参考图按重复 `image` form part 上传，备用模型由 `XG_FALLBACK_IMAGE_MODEL` 配置。
...
- 任务生图请求和结果图下载遇到 timeout 时自动重试 3 次；非 timeout 的配置错误和校验错误不得因为该规则被隐藏。统一生图 Gateway 的 Provider 响应错误在主路径重试耗尽后可进入已授权的 XG 备用 provider，若 XG 也失败，最终错误必须同时说明 Gateway 和 XG 的失败原因。
