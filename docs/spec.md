...
6. 提取抖音内容文案。
   ...
   - 如果作品是视频，后端从本地视频分离音频，再调用 SiliconFlow 音频多模态能力转录原始口播、旁白或对白。
   - 如果作品是图文，后端按图片顺序逐张使用本地 Python OCR SDK 提取图片中的文字，再合并为完整文案；OCR 完成后必须再次提交数据库，让详情页可以在故事总结仍在处理时先展示原始文案。
   - 如果作品是图文，后端把全部图片按顺序输入视觉理解模型生成故事总结，输出 `故事内容`、`故事爆点` 和 `目标观众`。
...
- 内容提取模型：图文图片文字提取使用本地 Python OCR SDK；图文故事总结和视频音频转写使用 SiliconFlow 多模态模型，通过 `/chat/completions` 传入 `image_url` 或 `audio_url` 内容；图文故事总结输入按下载顺序排列的全部图片，视频第一版先分离音频后转写，不直接做视频画面理解。
