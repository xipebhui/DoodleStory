...
产品必须原样保存用户输入的原始文本。完整故事模式下，LLM 负责根据时间线、连续场景、对白归属和情绪节奏切分原文并生成可画分镜，但每个 panel 的原文片段必须按顺序拼接后逐字等于用户原文，不能改写、摘要、补字或删字；图片内文字固定等于对应 panel 原文。故事方案模式下，LLM 直接根据用户方案规划图文分镜，不再先扩写成长篇故事再切分；规划概要必须与原始输入分开保存。提取分镜模式下，系统把内容提取结果结构化为可生图 panels，只保留和转化页序、画面、旁白、对白、内心 OS 和分格布局，不做故事扩写。人物对白必须写进 `visual_prompt`，和说话人物、动作、表情、对象绑定，不再作为 `image_text.dialogue` 单独字段传递，避免同一句台词重复入图。完整故事、故事方案、提取分镜和 DY 爆款复刻最终都要进入同一种页式分镜中间态：`第X页`、`【分格】`、`画面`、`旁白`、`对话`、`内心OS`；最终发送给图片模型的 prompt 再由 LLM 基于中间态、全局角色表、连续性规划、风格和参考图顺序统一编译。
...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 由 LLM 保持原文逐字覆盖地切分为 panels，并生成时间线、连续场景、对白归属和画面 prompt；`故事方案` 直接识别用户方案中的显式或隐式 panel 逻辑，并规划连续分镜、画面内容、分格布局和图片内文字；`提取分镜` 接收内容提取结果，按页结构化为 panels，不新增图片、不合并页、不扩写故事；`DY爆款复刻` 是任务创建前置模式，用户提交抖音分享文本或链接后，系统先执行内容提取，再自动用提取结果创建 `提取分镜` 生成任务。四种入口的后续底层处理都应统一到页式分镜中间态、任务级角色表、最终 prompt 编译和图片 job 链路。故事方案和提取分镜中用户明确写出的画面、构图、文字、重点和禁止事项优先于风格规则、默认分镜方法和剧情增强建议。
...
5. 执行任务生成。
   ...
   - 如果任务是完整故事模式，LLM 负责切分原文、识别倒叙/回忆/当前时间线、归纳连续场景、判断对白说话人并生成每个 panel 的 `visual_prompt`；后端必须校验所有 panel 原文片段拼接后逐字等于用户原文，不允许 LLM 改写或重构文本。
   - 如果用户选择固定图片数量，LLM 必须切成对应数量的原文 panels，所有 panels 拼接后必须逐字等于用户原文。
   ...
   - 完整故事模式的图文设计已经在 LLM storyboard planning 中生成，不再额外走程序 chunk 后的 panel prompt 生成。完整故事模式必须更克制：LLM 只能切分原文并设计画面，不允许新增、改写或概括 panel 原文，不允许把原文之外的对白、旁白、标题或字幕写进 prompt。
