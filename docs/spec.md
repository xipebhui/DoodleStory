...
产品必须原样保存用户输入的原始文本。完整故事模式下，程序只负责按断句边界切分原文，不允许 LLM 改写、摘要、补字或删字；图片内文字固定等于对应 panel 原文。故事方案模式下，LLM 直接根据用户方案规划图文分镜，不再先扩写成长篇故事再切分；规划概要必须与原始输入分开保存。提取分镜模式下，系统把内容提取结果结构化为可生图 panels，只保留和转化页序、画面、旁白、对白、内心 OS 和分格布局，不做故事扩写。人物对白必须写进 `visual_prompt`，和说话人物、动作、表情、对象绑定，不再作为 `image_text.dialogue` 单独字段传递，避免同一句台词重复入图。三种模式进入生图前都先统一为页式分镜中间态：`第X页`、`【分格】`、`画面`、`旁白`、`对话`、`内心OS`；最终发送给图片模型的 prompt 再由 LLM 基于中间态、全局角色表、风格和参考图顺序统一编译。
...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 保持原文切分；`故事方案` 直接识别用户方案中的显式或隐式 panel 逻辑，并规划连续分镜、画面内容、分格布局和图片内文字；`提取分镜` 接收内容提取结果，按页结构化为 panels，不新增图片、不合并页、不扩写故事；`DY爆款复刻` 是任务创建前置模式，用户提交抖音分享文本或链接后，系统先执行内容提取，再自动用提取结果创建 `提取分镜` 生成任务。故事方案和提取分镜中用户明确写出的画面、构图、文字、重点和禁止事项优先于风格规则、默认分镜方法和剧情增强建议。
...
5. 执行任务生成。
   ...
   - 任务级临时人物提取使用角色提取专用低温配置；完整故事模式下只把原始故事全文输入人物提取，不再重复传入所有 panels，避免长故事把上下文放大为“全文 + 切片全文”。当故事没有直接写外貌时，LLM 必须根据全文、称呼、人物关系、恋爱/亲情/校园/职场语境推断年龄阶段和性别呈现，例如第一人称“我”也要结合上下文推断为女学生、男孩、母亲等可统一形象，而不是输出“性别不明”的模糊人物。
   ...
   - 完整故事模式下，再次调用 LLM 时只输入当前批次 panel 原文和相邻 panel 的短上下文，只生成每张图的 `visual_prompt`；图片内文字不由 LLM 生成，固定使用该 panel 的原文片段，并在最终 prompt 编译输入中作为页式分镜中间态的 `旁白`。所选风格提示词进入该 LLM 调用的 system prompt，作为图像设计规则使用，不再作为普通 user 字段传入。
   ...
   - 正式生图前，后端必须调用最终 prompt 编译 LLM，把全局角色表、当前批次每页分镜中间态、图片内文字、风格信息和参考图顺序整理成每页完整画师指令。该编译层不再接收完整原文，当前页需要画出的故事内容以 panel 中间态为准；长故事按 `LLM_PANEL_BATCH_SIZE` 分批编译，但每批都必须携带同一份全局角色表，以解决角色外观与当前分镜、风格模板默认人物外观之间的冲突，并保证同一任务内固定角色和临时角色全局一致。
