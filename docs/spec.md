...
产品必须原样保存用户输入的原始文本。完整故事模式下，LLM 负责根据时间线、连续场景、对白归属和情绪节奏切分原文并生成可画分镜；“语义切分”只表示在场景变化、故事转折、动作变化或情绪变化处选择 panel 边界，不表示可以改写故事文本。每个 panel 的 `story_beat` 和图片内文字必须来自 `original_text` 中的连续原文片段，除换行和空白差异外必须覆盖完整原文，不能摘要、改写、润色、删句、补句、换称呼、压缩台词或合并不同位置的原文。故事方案模式下，LLM 直接根据用户方案规划图文分镜，不再先扩写成长篇故事再切分；规划概要必须与原始输入分开保存。提取分镜模式下，系统把内容提取结果结构化为可生图 panels，只保留和转化页序、画面、旁白、对白、内心 OS 和分格布局，不做故事扩写。人物对白必须写进 `visual_prompt`，和说话人物、动作、表情、对象绑定，不再作为 `image_text.dialogue` 单独字段传递，避免同一句台词重复入图。完整故事、故事方案、提取分镜和 DY 爆款复刻最终都要进入同一种页式分镜中间态：`第X页`、`【分格】`、`画面`、`旁白`、`对话`、`内心OS`；最终发送给图片模型的 prompt 再由 LLM 基于中间态、全局角色表、连续性规划、风格和参考图顺序统一编译。
...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 由 LLM 按语义自然切分为 panels，并生成时间线、连续场景、对白归属和画面 prompt；`故事方案` 直接识别用户方案中的显式或隐式 panel 逻辑，并规划连续分镜、画面内容、分格布局和图片内文字；`提取分镜` 接收内容提取结果，按页结构化为 panels，不新增图片、不合并页、不扩写故事；`DY爆款复刻` 是任务创建前置模式，用户提交抖音分享文本或链接后，系统先执行内容提取，再自动用提取结果创建 `提取分镜` 生成任务。四种入口的后续底层处理都应统一到页式分镜中间态、任务级角色表、最终 prompt 编译和图片 job 链路。故事方案和提取分镜中用户明确写出的画面、构图、文字、重点和禁止事项优先于风格规则、默认分镜方法和剧情增强建议。
- Panel：由用户原始故事、故事方案或内容提取结果规划出的画面片段。Panel 保存剧情意图、类型、图片内文字、生图画面 prompt 和分格布局；这些字段先形成页式分镜中间态，最终生图 prompt 由 LLM 统一编译为完整画师指令。
...
- 内容提取：用户粘贴抖音分享文本或链接，系统从中解析真实抖音 URL，下载图文或视频；图文把同一作品的全部图片按顺序一次性提交给 LIO 视觉理解模型，让模型结合前后页上下文后按页输出连贯漫画内容；视频通过音频多模态模型转写。
...
3. 管理角色。
   - 用户进入 `角色管理` tab。
   - 创建角色时填写名字并上传参考图；保存时不等待 VL，后端先保存角色和参考图，再后台调用 LIO 视觉模型自动生成可编辑的角色外观描述，并保存到角色 `description`。后台识别失败时最多重试 3 次，不阻塞用户保存。
...
4. 创建生成任务。
   - 用户注册或登录。
   - 通过居中弹窗创建任务。
   - 选择输入模式：完整故事、故事方案、提取分镜或 DY 爆款复刻。完整故事会保持故事不变，用户应只提交故事本身，不混入说明、标签或额外要求；故事方案可以提交故事设计、人物设定、画面要求或简化想法；提取分镜适合提交内容提取结果；DY 爆款复刻适合直接提交抖音分享文本或作品链接。前端创建弹窗检测到抖音分享链接时必须自动按 DY 爆款复刻提交到内容提取流程；后端普通任务创建接口必须拒绝抖音分享链接，不能把分享口令当成故事或分镜文本创建任务。
   - 输入原始文本、故事方案或创作要求。
   - 创建弹窗默认不展开固定角色绑定流程；普通用户不勾选 `使用固定角色` 时，点击 `创建任务` 直接提交任务，不等待角色提取。
   - 如果用户勾选 `使用固定角色`，底部主按钮先显示为 `提取角色`；系统通过后端接口同步调用 LIO 文本 LLM 提取角色名，提取完成后才展示角色卡片、添加角色入口和固定角色绑定控件，并允许用户继续创建任务。
...
5. 执行任务生成。
   ...
   - 任务前置分镜层统一使用 LLM：完整故事、故事方案和提取分镜都会先输出标题、规划概要、每个 panel 的剧情意图、画面 prompt、分格布局和图片内文字；DY 爆款复刻先经内容提取，再进入提取分镜的同一链路。
   - 如果任务是完整故事模式，LLM 负责切分原文、识别倒叙/回忆/当前时间线、归纳连续场景、判断对白说话人并生成每个 panel 的 `visual_prompt`；后端保留用户原始文本，并校验所有 panel 的 `story_beat` 拼接后除换行和空白差异外必须覆盖完整原文。LLM 只能选择自然语义边界，不能摘要、改写、润色、删句、补句或新增剧情。
   - 如果用户选择固定图片数量，LLM 必须刚好切成对应数量的 panels，并在数量限制内优先保持每页语义完整。
   - 故事方案模式下，固定图片数量就是最终图片张数，不额外插入特殊图片。
   - 如果未固定数量，LLM 按时间线、场景变化、对白归属、情绪节奏和画面承载量自动切分。
   ...
   - 任务级临时人物提取使用角色提取专用低温配置；人物提取在分镜层之后执行，可以读取原文和已经切好的 panels，从而输出不同年龄阶段 appearance 的 panel 绑定。完整故事的倒叙、回忆和多年后当下必须通过 panel 绑定区分 35 岁、53 岁等不同 appearance。当故事没有直接写外貌时，LLM 必须根据全文、称呼、人物关系、恋爱/亲情/校园/职场语境推断年龄阶段和性别呈现，例如第一人称“我”也要结合上下文推断为女学生、男孩、母亲等可统一形象，而不是输出“性别不明”的模糊人物。
   ...
   - 固定角色和临时角色都通过任务级角色提取或确定性匹配绑定到具体 panels；完整故事模式必须使用分镜层后的 panels 判断每格引用哪些角色和哪个年龄阶段。
   - 旧的纯自动人物参考不再替代用户固定角色；固定角色优先使用用户资产，未绑定主要人物使用任务内临时参考图。
   - 完整故事模式的图文设计已经在 LLM storyboard planning 中生成，不再额外走程序 chunk 后的 panel prompt 生成。完整故事模式必须更克制：LLM 只能围绕原文做语义切分和画面设计，不允许新增剧情、改变事实或把原文之外的对白、旁白、标题或字幕写进 prompt。
   - 故事方案和提取分镜模式下，图文设计同样已经在 storyboard planning 中生成，不再额外走 chunk 后的 panel prompt 生成。
   - 中间态 prompt 应描述主体、动作、场景状态、静态视觉内容和图片内文字在画面中的呈现方式，且已经吸收所选风格提示词的画风、人物、构图和文字规则。
   - 正式生图前，后端必须调用最终 prompt 编译 LLM，把全局角色表、任务级连续性规划、当前批次每页分镜中间态、图片内文字、风格信息和参考图顺序整理成每页完整画师指令。该编译层不再接收完整原文，当前页需要画出的故事内容以 panel 中间态为准；长故事按 `LLM_PANEL_BATCH_SIZE` 分批编译，但每批都必须携带同一份全局角色表和 compact storyboard context，以解决角色外观、时间线、连续场景、对白归属与风格模板默认人物外观之间的冲突，并保证同一任务内固定角色和临时角色全局一致。
   - 完整故事模式下，每张图片必须包含对应 panel 的图片内文字，文字来自 LLM 分镜后的 `story_beat`，不能额外添加“旁白”“字幕”“标题”等标签；如果原文直接引语已经在 `visual_prompt` 中绑定到人物说话动作，最终只画一次这句台词，使用对白气泡，旁白框不要重复同一句。故事方案模式下，每张图片必须包含图文设计中的图片内文字，并可按当前 panel 需要包含标题、旁白和内心 OS；人物对白从 `visual_prompt` 读取并画成对白气泡。提取分镜模式下，每张图片必须区分旁白框、对白气泡和内心 OS 思想气泡或心理独白框，但对白来源是 `visual_prompt`，不是 `image_text.dialogue`。最终 prompt 可以使用页级画师指令结构描述全域场景、分格、人物完整描述和文字位置，但必须明确中间态字段名只用于理解结构，不要把字段名画进图片。所有文字字号偏大、清晰可读。所有指定图片内文字只能出现一次，不能在上下分格、多栏或不同分屏里重复绘制同一段文字；如果是分格页面的整页旁白，最终生图 prompt 必须约束为只使用一个旁白框，除非文字列表已经明确拆成上格旁白、下格旁白等对应文字。文案应短、清楚、与画面强相关；旁白优先补充前因后果、人物关系、剧情信息或来自内容提取的原文，不只复述画面状态。
   - `prompt` 模式下，最终 prompt 编译 LLM 必须理解任务保存的 `style_prompt_snapshot`，先处理风格与角色、分镜之间的冲突；后端在发送图片 Provider 前还必须把完整 `style_prompt_snapshot` 显式拼接到最终生图 prompt 中，用于加强画风、人物比例、线条、色彩、构图、文字呈现和整体质感的一致性。该显式风格模板不得覆盖已锁定的固定角色或临时角色外观。所有正式 panel 生图和单 panel 修改最终发送给图片 Provider 的 prompt，第一行必须固定写明任务画面比例，并要求严格按该宽高比构图和出图；风格模板、参考图说明和最终画面指令都必须排在比例约束之后。`image` 模式下，最终 prompt 不再把大段 `style_prompt_snapshot` 作为风格来源，风格由任务快照中的参考图传入模型控制。完整故事模式的图片文字来自 LLM 分镜后的 `story_beat`；故事方案和提取分镜模式必须把分格布局、旁白和内心 OS 保存在结构字段中，人物对白必须写进 `visual_prompt`。
...
6. 查看和下载结果。
   ...
   - 开启人物参考时，任务详情只展示人物参考图、人物姓名和年龄/阶段，不展示内部人物分析、使用 notes 或完整人物设定；人物参考图可以点击放大预览，并支持下载或打开原图。
   - 点击图片放大预览。
...
7. 提取抖音内容文案。
   ...
   - 如果作品是视频，后端从下载服务返回的本地原始视频路径分离音频，再调用 LIO 音频多模态能力转录原始口播、旁白或对白。
   - 如果作品是图文，后端按图片顺序一次性把下载服务返回的本地原始图片路径输入 LIO 视觉理解模型，要求模型结合前后页上下文并按页输出旁白、对话、内心 OS、画面描述和分格信息；模型输出直接作为最终内容提取结果，并再次提交数据库。
...
- 内容提取模型：图文图片内容提取使用 LIO 多模态视觉模型，通过 `/chat/completions` 在同一次请求中按顺序传入全部 `image_url` 内容，要求模型结合前后图片保持叙事连贯，并按输入图片顺序逐页输出旁白、对话和内心 OS、画面描述和分格信息；图片素材登记为 DoodleStory 资产时必须使用抖音下载服务返回的原始文件 bytes，不做压缩、缩放或格式转换，并在对象存储上传完成后把资产公网原图 URL 按原顺序传给 VL，不再把图片转成 base64 data URL；没有公网 HTTP(S) URL 时必须明确失败。视频音频转写继续使用 LIO 多模态模型。内容提取和视频转写必须使用抖音下载服务返回的本地原始媒体路径，不应为了处理流程从对象存储公开 CDN 回拉刚下载的媒体。
...
- 创建任务时，不改写、不摘要、不清洗用户提交的源文本，必须原样保存用户源文本。完整故事模式的分镜文本只能按语义边界切割原文，除换行和空白差异外不得删改、摘要、润色或新增内容。
...
- Panel prompt 生成必须在 LLM system prompt 层遵守所选风格提示词；最终 panel 生图 prompt 必须由最终编译 LLM 基于任务保存的风格、全局角色表、参考图顺序和页式分镜中间态生成，不能只做代码字符串拼接。`prompt` 风格参考模式下，LLM 编译结果外层仍必须显式拼接任务保存的风格提示词快照，让图片模型直接执行风格规则；该拼接只增强风格一致性，不允许覆盖角色外观锁定。人物参考图 prompt 仍必须显式拼接任务保存的风格提示词快照。
...
- 完整故事模式下，LLM 分镜后的 `story_beat` 需要作为图片内可读文字进入最终生图 prompt，不能额外添加“旁白”“字幕”“标题”等标签；直接引语可以通过 `visual_prompt` 绑定到人物说话动作并画成对白气泡，但不能在旁白框重复。故事方案和提取分镜模式下，旁白、来自 `visual_prompt` 的人物对白和内心 OS 需要在 prompt 中区分呈现方式。最终 prompt 编译时可以读取 `旁白`、`对话`、`内心OS` 等中间态字段，但字段名只是结构化指令，不得画进图片。
...
- 抖音下载服务不可达、下载失败、下载后没有媒体文件、视频音频分离失败或 LIO 文案提取失败时必须明确报错，不能静默跳过媒体、自动改写文案或返回占位结果。
