...
产品必须原样保存用户输入的原始文本。完整故事模式下，程序只负责按断句边界切分原文，不允许 LLM 改写、摘要、补字或删字；图片内文字固定等于对应 panel 原文。故事方案模式下，LLM 直接根据用户方案规划图文分镜，不再先扩写成长篇故事再切分；规划概要必须与原始输入分开保存。提取分镜模式下，系统把内容提取结果结构化为可生图 panels，只保留和转化页序、画面、旁白、对白、内心 OS 和分格布局，不做故事扩写。三种模式进入生图前都先统一为页式分镜中间态：`第X页`、`【分格】`、`画面`、`旁白`、`对话`、`内心OS`；最终发送给图片模型的 prompt 再由 LLM 基于中间态、全局角色表、风格和参考图顺序统一编译。
...
4. 创建生成任务。
   ...
   - 输入原始文本、故事方案或创作要求。
   - 创建弹窗的角色参考默认开启：即使用户不点击 `提取角色`，后续任务执行仍会提取主要人物并生成任务内临时角色参考图，用于本任务全局角色一致性。
   - 创建弹窗提供醒目的 `AI 提取角色` 操作，通过后端接口同步调用硅基流动 `Qwen/Qwen3.6-27B` 提取角色名；该操作只用于让用户提前看到角色名、删除误识别角色或绑定自己的固定角色，不是临时角色生成链路的前置条件。
...
5. 执行任务生成。
   ...
   - 正式生图前，后端必须调用最终 prompt 编译 LLM，把全局角色表、每页分镜中间态、图片内文字、风格信息和参考图顺序整理成每页完整画师指令。该编译层负责解决角色外观与当前分镜、风格模板默认人物外观之间的冲突，并保证同一任务内固定角色和临时角色全局一致。
   - 完整故事模式下，每张图片必须包含对应 panel 原文，文字必须逐字一致，不能额外添加“旁白”“字幕”“标题”等标签。故事方案模式下，每张图片必须包含图文设计中的图片内文字；封面包含标题和钩子，剧情图可以包含旁白、人物对白和内心 OS。提取分镜模式下，每张图片必须区分旁白框、对白气泡和内心 OS 思想气泡或心理独白框。最终 prompt 可以使用页级画师指令结构描述全域场景、分格、人物完整描述和文字位置，但必须明确中间态字段名只用于理解结构，不要把字段名画进图片。所有文字字号偏大、清晰可读。所有指定图片内文字只能出现一次，不能在上下分格、多栏或不同分屏里重复绘制同一段文字；如果是分格页面的整页旁白，最终生图 prompt 必须约束为只使用一个旁白框，除非文字列表已经明确拆成上格旁白、下格旁白等对应文字。文案应短、清楚、与画面强相关；旁白优先补充前因后果、人物关系、剧情信息或来自内容提取的原文，不只复述画面状态。
   - `prompt` 模式下，最终 prompt 编译 LLM 必须理解任务保存的 `style_prompt_snapshot`，先处理风格与角色、分镜之间的冲突；后端在发送图片 Provider 前还必须把完整 `style_prompt_snapshot` 显式拼接到最终生图 prompt 中，用于加强画风、人物比例、线条、色彩、构图、文字呈现和整体质感的一致性。该显式风格模板不得覆盖已锁定的固定角色或临时角色外观。`image` 模式下，最终 prompt 不再把大段 `style_prompt_snapshot` 作为风格来源，风格由任务快照中的参考图传入模型控制。完整故事模式的图片文字固定为 panel 原文；故事方案和提取分镜模式必须把分格布局、旁白、对白、内心 OS 分别保存在结构字段中。
...
- 完整故事模式下，Panel 原文需要作为图片内可读文字进入最终生图 prompt，不能删改文案内容，也不能添加“旁白”“字幕”“标题”等标签；故事方案和提取分镜模式下，旁白、对白和内心 OS 需要在 prompt 中区分呈现方式。最终 prompt 编译时可以读取 `旁白`、`对话`、`内心OS` 等中间态字段，但字段名只是结构化指令，不得画进图片。
