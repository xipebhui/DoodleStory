...
- Sprint 117 已开始把产品运行时 Skill 的正式事实来源迁移为数据库中的 `agent_skills` 与不可变 `agent_skill_versions`：用户只编辑名称、适用场景、纯文本正文和受控 Tool 白名单，不编写 JSON/YAML/代码；草稿用 revision 做乐观并发控制，发布使用幂等键创建递增版本并默认启用，历史版本只能读取或显式重新启用。系统 `idea-to-comic` 由原受控文件幂等种入数据库，普通用户只读且可复制为个人草稿；`agent_runs.skill_version_id` 固定准确发布版本，Skill 后续发布、切换或归档不得改变已有 Run。基础 Agent 后续只读取可用 Skill catalog，选定后加载固定版本；skill_version_id、name、version 和内容 hash 必须写入 AgentStep 与 MLflow trace。
