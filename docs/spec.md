...
- 希望把短故事文本转换成连续图片的创作者。
- 负责调试可复用图片风格、维护风格 prompt 和后台生成配置绑定的运营/维护者。
...
1. 管理风格。
   ...
   - 添加参考图片和风格基础信息。
   - 配置风格提示词。
   - 风格不暴露 provider key、model key 和默认参数；这些信息由后台生成配置维护。
...
1. 精确保留用户原始文本。
2. 通过风格基础信息、提示词、测试生成和后台生成配置引用，让风格调试明确且可重复。
...
- 暂不做组织、团队、项目空间或租户级隔离。
- 图片模型不作为独立模块管理；provider key、model key、API key 和默认参数由后台配置维护，不暴露给普通用户。
- 第一版不支持用户编辑生成 prompt。
...
- 风格除了名称、描述、参考图片、prompt、状态和后台生成配置引用外，还需要哪些元数据？
