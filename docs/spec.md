...
- 内容提取：用户粘贴抖音分享文本或链接，系统从中解析真实抖音 URL，下载图文或视频，并通过多模态模型提取原始文案；图文内容还可以基于全部图片生成故事总结。
...
6. 提取抖音内容文案。
   ...
   - 后端随后在同进程后台处理该记录：调用同机抖音下载服务下载图文或视频，并把下载媒体登记为 DoodleStory 资产。
   - 如果作品是视频，后端从本地视频分离音频，再调用 SiliconFlow 音频多模态能力转录原始口播、旁白或对白。
   - 如果作品是图文，后端按图片顺序逐张调用 SiliconFlow 视觉理解能力提取图片中的文字，再合并为完整文案。
...
- 内容提取模型：图片文字提取、图文故事总结和视频音频转写使用 SiliconFlow 多模态模型，通过 `/chat/completions` 传入 `image_url` 或 `audio_url` 内容；图文故事总结输入按下载顺序排列的全部图片，视频第一版先分离音频后转写，不直接做视频画面理解。
- 内容提取创建使用轻量后台处理：提交后立即保存记录并返回列表，后台完成解析下载、文案提取和图文故事总结；列表按需刷新处理状态，仍保留显式重新提取、重新总结接口用于用户在详情中重新执行，不引入外部队列或复杂状态机。
