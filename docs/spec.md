...
产品必须原样保存用户输入的原始文本。完整故事模式下，LLM 负责根据时间线、连续场景、对白归属和情绪节奏切分原文并生成可画分镜；为了获得更自然的语义切割，panel 的 `story_beat` 和图片内文字可以对原文做轻微整理，例如合并换行、补足上下文或去掉不影响理解的口语停顿，但不能改变事件顺序、人物关系、关键动作、关键物品、年龄阶段、时间锚点或直接引语含义。故事方案模式下，LLM 直接根据用户方案规划图文分镜，不再先扩写成长篇故事再切分；规划概要必须与原始输入分开保存。提取分镜模式下，系统把内容提取结果结构化为可生图 panels，只保留和转化页序、画面、旁白、对白、内心 OS 和分格布局，不做故事扩写。人物对白必须写进 `visual_prompt`，和说话人物、动作、表情、对象绑定，不再作为 `image_text.dialogue` 单独字段传递，避免同一句台词重复入图。完整故事、故事方案、提取分镜和 DY 爆款复刻最终都要进入同一种页式分镜中间态：`第X页`、`【分格】`、`画面`、`旁白`、`对话`、`内心OS`；最终发送给图片模型的 prompt 再由 LLM 基于中间态、全局角色表、连续性规划、风格和参考图顺序统一编译。
...
5. 执行任务生成。
   ...
   - 如果任务是完整故事模式，LLM 负责切分原文、识别倒叙/回忆/当前时间线、归纳连续场景、判断对白说话人并生成每个 panel 的 `visual_prompt`；后端保留用户原始文本，同时允许 LLM 为更好的语义切割对 panel 文本做轻微整理，但不得改变事实、顺序、人物关系、关键台词含义或新增剧情。
...
- 创建任务时，不改写、不摘要、不清洗用户提交的源文本，必须原样保存用户源文本。完整故事模式的分镜文本可由 LLM 为语义切分做轻微整理，但不得改变事实、顺序、人物关系、关键台词含义或新增剧情。
