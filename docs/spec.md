...
4. 创建生成任务。
   ...
   - 可选勾选 `去掉画面文字`；默认不勾选。勾选后不改变任务分镜、旁白、对白、音频或视频生成所需的结构化字段；但最终生图 prompt 编译输入不再把这些文字作为画面内文字传递，最终发送给图片模型的提示词必须以 `最高指令，图片中不能包含任何文字。` 开头，并且不能包含 `【文字】`、旁白写入、字幕框、标题字、对白气泡、内心 OS 或其他要求绘制可读文字的指令。
   - 选择风格。
