...
5. 执行任务生成。
   ...
   - 故事方案模式下，所有 panel 都按同一套剧情分镜逻辑处理，并区分旁白、人物对白和内心 OS；如果用户明确写出上格/下格、上下两部分、左右两部分、分屏、单页构图、字体不能过小、必须出现或不要出现等要求，对应要求必须写入相关 panel 的画面、图片文字设计或 `text_layout`。
   - 提取分镜模式下，调用 LLM 只把内容提取结果结构化为 panels；输入中的 `第X页` 对应同序号 panel，不自动新增图片，不合并页，不扩写剧情。旁白、对白、内心 OS 必须分别进入图片文字结构，分格/多栏信息必须进入 `text_layout`。
