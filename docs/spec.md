...
产品必须原样保存用户输入的原始文本。普通故事模式下，LLM 可以用于文本切分和图片生成 prompt 生成，但不能改写用户提交的源故事。故事方案模式下，LLM 可以额外生成一份增强故事；增强故事必须与原始输入分开保存。
...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 保持原文切分；`故事方案` 会先整理成更适合抖音图文传播的标题、钩子和完整故事。
- Panel：由用户原始故事或增强故事规划出的画面片段。Panel 保存文本、类型、旁白和对白，之后会关联生图 prompt。
...
4. 执行任务生成。
   - 调用 LLM，使用文本切分系统提示词。
   - 如果任务是故事方案模式，先调用 LLM 将用户输入整理成更抓人的短视频/图文故事，输出标题、钩子和完整增强故事。
   ...
   - 开启人物参考时，panel prompt 生成还需要标注当前 panel 涉及哪些人物外形阶段。
   - 生成 prompt 应描述主体、动作、场景状态、静态视觉内容和 panel 文案在图片中的呈现方式，不能与风格提示词冲突。
   - 每张图片必须包含对应 panel 的文案内容；封面包含标题和钩子，剧情图包含旁白和人物对白。文案内容必须完整保留，换行、层级、重点强调和排版方式可以由 LLM 或图片模型自行决定。
...
- 故事方案模式可以生成增强故事，但必须保留原始输入，并将增强故事保存在独立字段中。
- 故事方案模式如果增强失败，任务失败并显示明确错误，不能生成占位故事或静默切回普通模式。
