...
- 人物参考：任务级可选能力。开启后，系统会在切分故事后识别主要人物及其不同年龄阶段，先生成对应人物参考图，再把相关人物参考图按固定顺序传入每个 panel 的生图请求。人物参考图采用角色设定图布局：上半部分为正面主图，下半部分为同一人物的左侧和右侧两个侧视图。
...
0. 进入工作台。
   ...
   - 主工作台页面必须有稳定二级路径：任务页 `/tasks`、内容提取页 `/content-extractions`、风格页 `/styles`、管理员用户管理页 `/users`、管理员积分消耗页 `/credit-usage`、设置页 `/settings`。
...
4. 执行任务生成。
   ...
   - 开启人物参考时，先使用任务风格提示词生成每个人物年龄阶段的参考图；最终发送给图片模型的人物参考图 prompt 必须把 `style_prompt_snapshot` 作为独立风格提示词段显式拼接，强化人物画风一致性。人物参考图必须采用上方正面主图、下方两个侧视图的角色设定图布局，三张视图保持同一人物、同一服装、同一发型和同一稳定外观锚点。人物参考图只用于后续 panel 生图，不向用户展示内部人物提示词；风格参考图不参与人物参考图生成。
   - 人物参考图成功产出后按图片数扣积分，每张参考图扣 `1` 积分。
