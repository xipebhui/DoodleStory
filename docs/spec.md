...
4. 执行任务生成。
   ...
   - 完整故事模式下，再次调用 LLM 输入原始故事、所选风格和 panel 文本，生成每张图的图文设计：`visual_prompt`、`image_text` 和 `text_layout`。
   ...
   - 生成 prompt 应描述主体、动作、场景状态、静态视觉内容和图片内文字在画面中的呈现方式，不能与风格提示词冲突。
   - 每张图片必须包含图文设计中的图片内文字；封面包含标题和钩子，剧情图包含旁白和必要的人物对白。文案应短、清楚、与画面强相关，换行、层级、重点强调和排版方式可以由 LLM 或图片模型自行决定。
   - prompt 生成的系统消息必须强调遵守所选风格。
   - 开启人物参考时，每个 panel 生图请求必须按“人物参考图在前、风格参考图在后”的顺序传入参考图，并在最终 prompt 中声明“参考图 1/2/...”对应的人物身份和阶段。
