...
4. 执行任务生成。
   ...
   - 每张图片必须包含图文设计中的图片内文字；封面包含标题和钩子，剧情图包含旁白和必要的人物对白。文案应短、清楚、与画面强相关；旁白优先补充前因后果、人物关系或剧情信息，不只复述画面状态。
   - 最终发送给图片模型的 panel 生图 prompt 不再拼接原始风格提示词，而是使用已风格化的图文设计，并以自然分段组织为参考、画面比例、画面和图片文字。人物动作、状态和对白必须合并在画面段中，不再拆成独立对白或排版结构。
