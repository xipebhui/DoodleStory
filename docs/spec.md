...
- 任务：用户发起的一次文本转图片请求。
- 输入模式：`完整故事` 保持原文切分；`故事方案` 直接识别用户方案中的显式或隐式 panel 逻辑，并规划封面、剧情图、画面内容和图片内文字；`提取分镜` 接收内容提取结果，按页结构化为 panels，不新增封面、不合并页、不扩写故事；`DY爆款复刻` 是任务创建前置模式，用户提交抖音分享文本或链接后，系统先执行内容提取，再自动用提取结果创建 `提取分镜` 生成任务。故事方案和提取分镜中用户明确写出的画面、构图、文字、重点和禁止事项优先于风格规则、默认分镜方法和剧情增强建议。
...
4. 执行任务生成。
   ...
   - 完整故事模式下，每张图片必须包含对应 panel 原文，文字必须逐字一致，不能额外添加“旁白”“字幕”“标题”等标签。故事方案模式下，每张图片必须包含图文设计中的图片内文字；封面包含标题和钩子，剧情图可以包含旁白和人物对白。提取分镜模式下，每张图片必须区分旁白框、对白气泡和内心 OS 思想气泡或心理独白框；最终生图 prompt 的图片文字段必须使用“以旁白框呈现 / 以对白气泡呈现”等自然说明，不输出 `旁白：`、`对白：`、`内心OS：` 这类容易被画进图片的类型标签；单页漫画构图不输出单独布局块，分格或多栏要求只作为画面构图指令写入，不输出 `分格/多栏布局：` 这类结构标题。所有文字字号偏大、清晰可读。所有指定图片内文字只能出现一次，不能在上下分格、多栏或不同分屏里重复绘制同一段文字；如果是分格页面的整页旁白，最终生图 prompt 必须约束为只使用一个旁白框，除非文字列表已经明确拆成上格旁白、下格旁白等对应文字。文案应短、清楚、与画面强相关；旁白优先补充前因后果、人物关系、剧情信息或来自内容提取的原文，不只复述画面状态。
   - `prompt` 模式下，最终发送给图片模型的 panel 生图 prompt 必须显式拼接所选风格的 `style_prompt_snapshot`，并以自然分段组织为参考、风格提示词、画面比例、画面和图片文字。`image` 模式下，最终 prompt 不再把大段 `style_prompt_snapshot` 作为风格来源，风格由任务快照中的参考图传入模型控制。完整故事模式的图片文字固定为 panel 原文；故事方案模式的人物动作、状态和对白必须合并在画面段中，不再拆成独立对白或排版结构。
...
7. 管理积分和用户。
   - 普通用户进入设置页后，可以查看当前积分余额、占用积分、最近积分流水和使用说明。
   - 普通用户可以在设置页输入激活码兑换积分；兑换成功后余额立即刷新，并写入积分流水。
   - Admin 进入设置页后，可以查看用户列表、用户积分余额、任务数量、成功图片数、消耗积分和注册时间。
   - Admin 可以手动增减用户积分，但必须填写原因；调整结果写入积分流水并记录管理员操作者。
   - Admin 可以生成固定面额的激活码，并可查看激活码状态、兑换人和兑换时间。
