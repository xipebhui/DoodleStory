...
当前市场扫描、评分、深挖样本选择、评论/账号/VL 探测、topic_hypothesis、experiment_plan、H1/H2 源样本 `full_story_extract` 和 4 个发布槽的 `generation_brief` 均已完成。第一轮实验不再泛测整个关键词，只验证 `family_marriage` 下两个机制：
...
H1 源样本 `7649315939447871470` 已完成 8 页完整图集 VL 提取，H2 源样本 `7650413089900236066` 已完成 15 页完整图集 VL 提取，并已为 4 个发布槽分别写出可进入 DoodleStory `故事方案` 模式的原创 `generation_brief`。这些证据只能用于分析结构和原创改写，不能直接照搬源故事桥段、人物关系或具体生活细节。H1 后续只能提炼“家庭身份规则循环”机制，不能沿用年轻后妈、后后妈、未成年人或暧昧继亲关系等高风险桥段。

发布前还必须完成：

1. 创建真实生成任务并回填 `content_id` 或 `task_id`。
2. 回填绝对 `planned_publish_time`。
3. 根据素材完成和审核情况确认是否启用第 3 条加速窗口。
