...
当前市场扫描、评分、深挖样本选择、评论/账号/VL 探测、topic_hypothesis、experiment_plan 和 H2 源样本 `full_story_extract` 均已完成。第一轮实验不再泛测整个关键词，只验证 `family_marriage` 下两个机制：
...
H2 源样本 `7650413089900236066` 已完成 15 页完整图集 VL 提取，可进入 `generation_brief`。该证据只能用于分析结构和原创改写，不能直接照搬源故事桥段、人物关系或具体生活细节。

发布前还必须完成：

1. 完成 H2 `generation_brief`，形成 DoodleStory 原创故事方案。
2. 完成 H1 `full_story_extract` 或确认 H1 本轮先不生成。
3. 创建真实生成任务并回填 `content_id` 或 `task_id`。
4. 回填绝对 `planned_publish_time`。
5. 根据素材完成和审核情况确认是否启用第 3 条加速窗口。
