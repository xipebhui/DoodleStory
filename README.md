...
DoodleStory 是一个文本转图片的故事生成项目。它会把用户输入的原始文本切分成一组画面片段，再结合风格库、风格提示词和风格内配置的图片模型生成多张图片。风格控制以模板提示词为主，风格参考图只作为样张展示。

## 产品形态

- 用户系统：支持邮箱注册、登录、退出和找回密码；普通用户只能看到自己的任务，Admin 可以看到全部任务。
- 风格库：管理图片风格、样张参考图片、风格基础信息和风格提示词；provider、model 和默认参数由后台生成配置维护，不暴露给普通用户。
